...
let update msg model =
    ...
    | SaveTodo msg ->
        ...
        | Finished todos ->
            ...
            Cmd.none
    | ClearTodos -> model, Cmd.OfAsync.perform todosApi.clearTodos () GotTodos
    | GotTodos todos -> { model with Todos = Loaded todos }, Cmd.none
    ...
    let todoAction model dispatch =
        Html.div [
            ...
            prop.children [
                ...
                Html.button [
                    prop.className
                        "flex-no-shrink p-2 px-12 rounded bg-yellow-600 outline-none focus:ring-2 ring-teal-300 font-bold text-white hover:bg-teal disabled:opacity-30 disabled:cursor-not-allowed text-sm sm:text-base"
                    prop.disabled (
